...
#
# Usage:
#   ./scripts/bench_claude_code_max.sh           # runs both variants, 1 run each
#   RUNS=3 ./scripts/bench_claude_code_max.sh    # 3 runs per variant per prompt
#
# Output:
#   evals/runs/claude_code_max/{plain,flint}_r<i>.jsonl
...
cmd = sys.argv[1]
out_path = sys.argv[2]
corpus = [json.loads(l) for l in open("evals/claude_code_max_prompts.jsonl") if l.strip()]
